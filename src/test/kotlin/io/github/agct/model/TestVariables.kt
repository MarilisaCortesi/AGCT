...
import io.github.agct.model.variables.DEFAULT_CONCENTRATION_VALUE
import io.github.agct.model.variables.DEFAULT_RATE_VALUE
import io.github.agct.shouldEqual
import io.github.agct.shouldMatchWithTolerance
...
internal class TestVariables : StringSpec({
    "test rate" {
        ...
        val arrayRate =
            BasicRate(*MULTIPLE_DOUBLE_ELEMENTS.toDoubleArray())
        arrayRate.values shouldMatchWithTolerance MULTIPLE_DOUBLE_ELEMENTS
        val sequenceRate =
            BasicRate(MULTIPLE_DOUBLE_ELEMENTS.asSequence())
    ...
    "test concentration" {
        ...
        val singleConcentration =
            BasicConcentration(SINGLE_INTEGER_ELEMENT)
        singleConcentration.values shouldHaveSingleElement SINGLE_INTEGER_ELEMENT
        val arrayConcentration =
            BasicConcentration(*MULTIPLE_INTEGER_ELEMENTS.toIntArray())
        arrayConcentration.values shouldMatchWithTolerance MULTIPLE_INTEGER_ELEMENTS
        val sequenceConcentration =
            BasicConcentration(MULTIPLE_INTEGER_ELEMENTS.asSequence())
        sequenceConcentration.values shouldMatchWithTolerance MULTIPLE_INTEGER_ELEMENTS
        val setConcentration =
            BasicConcentration(MULTIPLE_INTEGER_ELEMENTS)
        setConcentration.values shouldMatchWithTolerance MULTIPLE_INTEGER_ELEMENTS
    }

    "test equality" {
        BasicRate(MULTIPLE_DOUBLE_ELEMENTS) shouldEqual BasicRate(
            MULTIPLE_DOUBLE_ELEMENTS
        )
        BasicConcentration(MULTIPLE_INTEGER_ELEMENTS) shouldEqual BasicConcentration(
            MULTIPLE_INTEGER_ELEMENTS
        )
        BasicRate(MULTIPLE_DOUBLE_ELEMENTS) shouldNotEqual BasicConcentration(
            MULTIPLE_INTEGER_ELEMENTS
        )
    }
